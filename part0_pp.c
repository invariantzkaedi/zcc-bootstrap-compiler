...
static const char *zcc_stddef_text =
    ...
    "typedef unsigned long __uint128_t;\n"
    "typedef long __int128_t;\n"
    "typedef char int8_t;\n"
    ...
    "typedef long jmp_buf[25];  /* x86-64 glibc: __jmp_buf(64) + mask_saved(4) "
    "+ sigset(128) + pad = 200 bytes */\n"
    ...
    "/* PP-STUB-SATELLITE: libc prototypes for ZCC satellite file compilation */\n"
    ...
    "int tolower(int c);\n"
    "int toupper(int c);\n"
    "/* PP-STUB-SATELLITE: POSIX filesystem/socket primitives */\n"
    ...
    "/* PP-STUB-SATELLITE: fcntl extras */\n"
    "int fcntl(int fd, int cmd, ...);\n"
    "#define F_GETFL 3\n"
    "#define F_SETFL 4\n"
    "#define O_NONBLOCK 2048\n"
    "/* PP-STUB-SATELLITE: stat */\n"
    "struct stat { unsigned long st_dev; unsigned long st_ino; unsigned int st_mode; unsigned long st_nlink; unsigned int st_uid; unsigned int st_gid; unsigned long st_rdev; long st_size; long st_blksize; long st_blocks; long st_atime; long st_mtime; long st_ctime; };\n"
...
static const char *zcc_simd_text =
    ...
    "__m256i _mm256_loadu_si256(const __m256i *p);\n"
 ...
 * Called before stream-continuation loop guards and by pp_next after pos
...
static void pp_drain_frames(PPState *state) {
  while (state->pos >= state->len && state->input_depth > 0) {
    ...
    for (int bi = 0; bi < state->num_blocked; bi++) {
      state->blocked_macros[bi] =
          state->input_stack[state->input_depth].blocked_macros[bi];
    }
  ...
  if (++pp_peek_cnt % 500000 == 0) {
    if (getenv("ZCC_DEBUG"))
      printf("DEBUG zcc2 pp_peek_cnt=%d pos=%d line=%d\n", pp_peek_cnt,
             state->pos, state->line);
    fflush(stdout);
  }
  if (state->pos >= state->len)
    return 0;
  if (state->src[state->pos] == '\r')
    return '\n';
...
static void pp_push_input(PPState *state, const char *new_src, char *alloc_buf,
                          PPMacro *macro) {
  ...
  if (macro) {
    int found = 0;
    for (int bi = 0; bi < state->num_blocked; bi++) {
      if (state->blocked_macros[bi] == macro) {
        found = 1;
        break;
      }
    }
    if (!found) {
      if (state->num_blocked >= PP_MAX_HIDESET) {
        fprintf(
            stderr,
            "zcc: FATAL: macro hide-set overflow (depth > %d) near macro '%s' "
...
static void pp_parse_ident(PPState *state, char *out, int max) {
  ...
  while (is_ident_char(pp_peek(state)) && i < max - 1)
    out[i++] = pp_next(state);
  out[i] = 0;
...
static int is_empty_fallback_safe(const char *path) {
  ...
  /* Null-byte injection guard: verify no null-byte exists in path before true
   * end */
  int len_p = 0;
  while (path[len_p] != '\0')
    len_p++;
  ...
  if (len >= 8 && strcmp(base + len - 8, "config.h") == 0)
    return 1;
  if (len >= 7 && strcmp(base + len - 7, "setup.h") == 0)
    return 1;

  /* 3. Strict Hardcoded Whitelist (zlib.h removed) */
  return strcmp(base, "libbb.h") == 0 || strcmp(base, "git-compat-util.h") == 0;
}
...
static int is_stddef_stub(const char *path) {
  ...
  if (slash)
    base = slash + 1;
  return strcmp(base, "stddef.h") == 0 || strcmp(base, "stdarg.h") == 0 ||
         strcmp(base, "stdio.h") == 0 || strcmp(base, "stdlib.h") == 0 ||
         strcmp(base, "string.h") == 0 || strcmp(base, "math.h") == 0 ||
         strcmp(base, "errno.h") == 0 || strcmp(base, "fcntl.h") == 0 ||
         strcmp(base, "setjmp.h") == 0 || strcmp(base, "time.h") == 0 ||
         strcmp(base, "unistd.h") == 0 || strcmp(base, "dlfcn.h") == 0 ||
         ...
         /* PP-STUB-SATELLITE: POSIX headers for satellite file compilation */
         strcmp(base, "ctype.h") == 0 || strcmp(base, "limits.h") == 0 ||
...
static int pp_resolve_path(PPState *state, const char *path, int is_system,
                           char *out_resolved) {
  char dir[1024];
  char seg[1024];
  const char *sl;
  ...
  if (state->include_paths && state->include_paths[0]) {
    ...
    while (*p) {
      ...
      while (*p && *p != ':' && si < 1022)
        seg[si++] = *p++;
      seg[si] = '\0';
      /* CG-PP-001: strip trailing slash — prevents double-slash
         on fopen when -I path has trailing / (e.g. -Iinclude/) */
      while (si > 1 && seg[si - 1] == '/')
        seg[--si] = '\0';
      if (*p == ':')
        p++;
...
static void pp_process_include(PPState *state, const char *path,
                               int is_system) {
  char *file_src;
  int file_len;
  int i;
  ...
  if (!sub_state) {
    fprintf(stderr,
            "zcc preprocessor error: out of memory for include state\n");
    exit(1);
  }
  ...
  for (i = state->num_macros; i < sub_state->num_macros; i++) {
    ...
    memcpy(&state->macros[state->num_macros++], &sub_state->macros[i],
           sizeof(PPMacro));
  }

  fprintf(stderr, "pp_process_include: %s added %d macros, total is %d\n", path,
          state->num_macros - initial_macros, state->num_macros);
  ...
  while (pp_peek(state) != ')' && pp_peek(state) != '\n' &&
    ...
    if (is_ident_start(pp_peek(state))) {
      if (m->num_params < PP_MAX_PARAMS) {
        ...
        if (!_warned_pp_max_params) {
          printf("zcc: warning: PP_MAX_PARAMS (%d) exceeded at %s:%d — "
                 "subsequent macro params silently discarded\n",
                 PP_MAX_PARAMS, state->filename ? state->filename : "?",
                 state->line);
          _warned_pp_max_params = 1;
        }
...
static long long pp_e_primary(PPEval *e) {
  ...
  if (*e->s >= '0' && *e->s <= '9') {
    ...
    while (*e->s == 'u' || *e->s == 'U' || *e->s == 'l' || *e->s == 'L')
      e->s++;
    return v;
  }
  /* identifier: defined() or macro */
  if ((*e->s >= 'a' && *e->s <= 'z') || (*e->s >= 'A' && *e->s <= 'Z') ||
      *e->s == '_') {
    char name[256];
    int n = 0;
    ...
    name[n] = 0;
    if (n == 7 && name[0] == 'd' && name[1] == 'e' && name[2] == 'f' &&
        name[3] == 'i' && name[4] == 'n' && name[5] == 'e' && name[6] == 'd') {
      pp_e_skip(e);
      int paren = 0;
      if (*e->s == '(') {
        e->s++;
...
static long long pp_e_mul(PPEval *e) {
  ...
  for (;;) {
    ...
    if (pp_e_match(e, "*"))
      v = v * pp_e_primary(e);
    else if (pp_e_match(e, "/")) {
      long long r = pp_e_primary(e);
      v = (r == 0 || (v == -9223372036854775807LL - 1LL && r == -1)) ? 0
                                                                     : v / r;
    } else if (pp_e_match(e, "%")) {
      long long r = pp_e_primary(e);
      v = (r == 0 || (v == -9223372036854775807LL - 1LL && r == -1)) ? 0
...
static long long pp_e_shift(PPEval *e) {
  ...
  for (;;) {
    pp_e_skip(e);
    if (pp_e_match(e, "<<"))
      v = (long long)((unsigned long long)v << pp_e_add(e));
    else if (pp_e_match(e, ">>"))
      v = (long long)((unsigned long long)v >> pp_e_add(e));
    else
      break;
  ...
  if (strcmp(dir, "ifdef") == 0 || strcmp(dir, "ifndef") == 0) {
    ...
    if (!active) {
      ...
      if (strcmp(dir, "ifdef") == 0)
        state->cond_stack[state->cond_depth] = is_defined;
      else
        state->cond_stack[state->cond_depth] = !is_defined;
    }
    ...
    if (strcmp(dir, "define") == 0) {
      ...
      /* Strip trailing \r from Windows line endings */
      {
        ...
        while (plen > 0 &&
               (inc_path[plen - 1] == '\r' || inc_path[plen - 1] == ' ' ||
                inc_path[plen - 1] == '\t'))
          inc_path[--plen] = 0;
        if (plen > 0 &&
            (inc_path[plen - 1] == '"' || inc_path[plen - 1] == '>'))
          inc_path[--plen] = 0;
      }
...
static void pp_expand_ident(PPState *state, const char *ident) {
  ...
  /* ATTR-UNKNOWN-001: handle __attribute__ specially — intercept BEFORE macro
   * lookup so the empty #define __attribute__(x) never fires for this
   * identifier. We parse (( attrname )) directly from the stream and emit
  ...
  if (strcmp(ident, "__attribute__") == 0 ||
    ...
    /* read attribute name */
    {
      ...
      /* strip __ prefix/suffix (GCC __packed__ etc.) */
      {
        char *an = attr_name;
        int alen = ai;
        if (alen > 4 && an[0] == '_' && an[1] == '_' && an[alen - 1] == '_' &&
            an[alen - 2] == '_') {
          an[alen - 2] = 0;
          an += 2;
        }
        if (strcmp(an, "packed") == 0) {
          ...
          if (pp_peek(state) == '(') {
            ...
            if (n > 0) {
              if (n < 1 || n > 8192) {
                fprintf(stderr,
                        "zcc preprocessor error: aligned(N) value %d out of "
                        "sane range (1..8192)\n",
                        n);
              } else {
                char buf[64];
  ...
  while (pp_peek(state) != 0) {
    ...
    if (c == '\\') {
      ...
      if (p_count < PP_MAX_PARAMS) {
        ...
        if (!_warned_pp_max_params && p_count >= PP_MAX_PARAMS) {
          printf("zcc: warning: PP_MAX_PARAMS (%d) exceeded at %s:%d — "
                 "subsequent macro args silently discarded\n",
                 PP_MAX_PARAMS, state->filename ? state->filename : "?",
                 state->line);
          _warned_pp_max_params = 1;
        }
  ...
  for (i = 0; i < len; i++) {
    ...
    if (m->body[i] == '#' && m->body[i + 1] != '#') {
      int tmp_i = i + 1;
      while (tmp_i < len && (m->body[tmp_i] == ' ' || m->body[tmp_i] == '\t'))
        tmp_i++;
...
static void pp_parse_target_depth(PPState *state, int target_depth) {
  ...
  while (1) {
    pp_drain_frames(
        state); /* PP-001: drain before each peek in main scan loop */
    char c = pp_peek(state);
    if (state->input_depth < target_depth || c == 0)
      break;

    if (c == '"' || c == '\'') {
      in_string = c;
      if (pp_is_active(state))
        pp_emit(state, pp_next(state));
      else
        pp_next(state);
    ...
    if (c == '/' && state->pos + 1 < state->len) {
      ...
      if (next_c == '/') {
        ...
        while (pp_peek(state) != 0) {
          if (pp_peek(state) == '*' && state->pos + 1 < state->len &&
...
char *zcc_preprocess(const char *source, int source_len, const char *filename,
                     const char *include_paths, const char *define_flags,
                     int *out_len) {
