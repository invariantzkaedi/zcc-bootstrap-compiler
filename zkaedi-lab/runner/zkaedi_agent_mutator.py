...
# Refactored mutation search runner for trajectory stability and multi-seed harness execution
def run_mutation_search(seed, base_model, dpo_model, base_tokenizer, dpo_tokenizer, seed_cand, evaluated_cache, pareto_archive, generations=5, verbose=True):
    np.random.seed(seed)
    ...
    for gen in range(generations):
        if verbose:
            print(f"\n--- Generation {gen + 1} / {generations} (Seed {seed}) ---")
        ...
        # Translate coordinates q to prompt and scaffold parameters
        if q[0] < -0.3:
        ...
        # Calculate continuous normalized RBF potential V0(q) and continuous gradients
        eps_reg = 1e-5
        weights = []
        grad_weights = []
        ...
        # update pareto list inline
        # Modifies list passed in place or returns updated list
        # We explicitly update the reference
...
def main():
    ...
    if os.path.exists(cache_path):
        try:
            ...
            print(f"Loaded {len(evaluated_cache)} cached execution entries.")
        except Exception as e:
    ...
    if os.path.exists(pareto_path):
        try:
            ...
            print(f"Loaded Pareto Archive with {len(pareto_archive)} entries.")
        except Exception as e:
            print(f"Warning: Failed to load Pareto Archive: {e}")
            
    # Execute mutation search trajectory
    history = run_mutation_search(
        seed=RANDOM_SEED,
