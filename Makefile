CFLAGS = -O0 -w -fno-asynchronous-unwind-tables -g0 -DZCC_REAL_TELEMETRY -Iinclude -I.
LDFLAGS = -lm
ifneq ($(NO_STRIP),1)
LDFLAGS += -Wl,-s
endif
...
selfhost-tablet: verify-lexicon zcc
	python3 zcc_quest.py --cmd "bash gate.sh" --mode tablet

selfhost-enki: verify-lexicon zcc
	python3 zcc_quest.py --cmd "bash gate.sh" --mode enki
...
.PHONY: check-qasm check-qasm-opt
check-qasm: zcc
	@echo "=== [1/4] Running OpenQASM 2.0 Parser Test Suite ==="
	@python3 tests/test_qasm_parser.py
	@echo "=== [2/4] Running OpenQASM 2.0 Statevector Simulator Test Suite ==="
	@python3 tests/test_qasm_sim.py
	@echo "=== [3/4] Running OpenQASM 2.0 Circuit Optimizer Test Suite ==="
	@python3 tests/test_qasm_opt.py
	@echo "=== [4/4] Running C API Native Quantum Tests ==="
	@gcc -O2 -Iinclude -I. -o /tmp/test_qasm_c_api tests/test_qasm_c_api.c src/quantum/zcc_qasm_parser.c src/quantum/zcc_qasm_sim.c src/quantum/zcc_qasm_opt.c -lm
	@/tmp/test_qasm_c_api
