...
/* ZCC Resource Oracle telemetry global declarations */
extern void zcc_oracle_init(const char *source_file);
extern void zcc_oracle_shutdown(void);
extern void zcc_oracle_log_pass(const char *pass_name, const char *function_name, 
                                double duration_ms, size_t heap_bytes, 
                                int spills, int virtual_regs, int physical_regs);
extern void zcc_oracle_log_sentinel(size_t idt_size, size_t gdt_size, 
...
extern void zcc_serialize_ast_json(FILE *out, Compiler *cc, Node *prog);
...
int zcc_main(int argc, char **argv) {
  ...
  /* Initialize prefix mapping to normalize compile paths (cross-run determinism) */
  {
    char cwd_buf[1024];
    if (getcwd(cwd_buf, sizeof(cwd_buf)) != NULL) {
      char escaped_cwd[2048];
      int src_idx = 0, dst_idx = 0;
      while (cwd_buf[src_idx] && dst_idx < sizeof(escaped_cwd) - 10) {
        if (cwd_buf[src_idx] == '\'') {
          strcpy(escaped_cwd + dst_idx, "'\\''");
          dst_idx += 4;
        } else {
          escaped_cwd[dst_idx++] = cwd_buf[src_idx];
        }
        src_idx++;
      }
      escaped_cwd[dst_idx] = '\0';
      int written = snprintf(prefix_map, sizeof(prefix_map),
               "-fdebug-prefix-map='%s'=. -ffile-prefix-map='%s'=. -fmacro-prefix-map='%s'=.",
               escaped_cwd, escaped_cwd, escaped_cwd);
      if (written < 0 || (size_t)written >= sizeof(prefix_map)) {
        prefix_map[0] = '\0';
      }
    }
  }
  ...
  int qasm_sim_mode = 0;
  int qasm_opt_mode = 0;
  int qasm_sim_c_mode = 0;
  int qasm_clifford_t_mode = 0;
  ...
  for (i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--target=wasm32-wasi") == 0 || strcmp(argv[i], "--target=wasm32") == 0) {
      ...
      qasm_opt_mode = 1;
    } else if (strcmp(argv[i], "--target=qasm-clifford-t") == 0 || strcmp(argv[i], "--qasm-clifford-t") == 0) {
      qasm_clifford_t_mode = 1;
    } else if (strcmp(argv[i], "--target=qasm-sim-c") == 0 || strcmp(argv[i], "--qasm-sim-c") == 0) {
      qasm_sim_c_mode = 1;
      ...
      setenv("ZCC_USE_PROFILE", argv[i] + 14, 1);
    } else if (strcmp(argv[i], "-fprofile-use") == 0) {
      i++;
      if (i < argc) setenv("ZCC_USE_PROFILE", argv[i], 1);
      ...
      if (len > 2 && (strcmp(argv[i] + len - 2, ".c") == 0 || (len > 4 && strcmp(argv[i] + len - 4, ".cpp") == 0) || strcmp(argv[i] + len - 3, ".rs") == 0 || (len > 4 && strcmp(argv[i] + len - 4, ".yul") == 0) || (len > 5 && strcmp(argv[i] + len - 5, ".html") == 0) || (len > 4 && strcmp(argv[i] + len - 4, ".css") == 0) || (len > 4 && strcmp(argv[i] + len - 4, ".svg") == 0) || (len > 7 && strcmp(argv[i] + len - 7, ".circom") == 0) || (len > 5 && strcmp(argv[i] + len - 5, ".qasm") == 0) || (qasm_canonical_mode && argv[i][0] != '-') || (qasm_validate_mode && argv[i][0] != '-') || (qasm_sim_mode && argv[i][0] != '-') || (qasm_opt_mode && argv[i][0] != '-') || (qasm_clifford_t_mode && argv[i][0] != '-') || (qasm_sim_c_mode && argv[i][0] != '-'))) {
        input_file = argv[i];
      } else {
        if (extra_link_args[0]) strncat(extra_link_args, " ", 4095 - (int)strlen(extra_link_args));
        strncat(extra_link_args, argv[i], 4095 - (int)strlen(extra_link_args));
  ...
  if (!input_file) {
    if (extra_link_args[0] && output_file) {
        ...
        int written = snprintf(cmd, sizeof(cmd),
            "gcc %s -O0 -w -no-pie -fno-asynchronous-unwind-tables -Wa,--noexecstack -fno-unwind-tables -o %s %s -lm -lpthread -ldl 2>&1",
            prefix_map, output_file, extra_link_args);
        if (written < 0 || (size_t)written >= sizeof(cmd)) {
            fprintf(stderr, "zcc: link command too long\n");
            return 1;
        }
        return system(cmd);
    }
  ...
  int explicit_output_file = (output_file != NULL);
  if (!output_file)
    output_file = "a.out";
  ...
  if (strlen(input_file) >= 5 && strcmp(input_file + strlen(input_file) - 5, ".html") == 0) {
    ...
    frontend_lang = FRONTEND_LANG_ASSET;
  } else if (strlen(input_file) >= 7 && strcmp(input_file + strlen(input_file) - 7, ".circom") == 0) {
    frontend_lang = FRONTEND_LANG_ASSET;
  ...
  zcc_oracle_init(input_file);
  atexit(zcc_oracle_shutdown);
  if (g_emit_zxr_path || g_replay_zxr_path || enable_telemetry_stdout) {
      lucky_alert_emit(ALERT_TIP | ALERT_IMPORTANT | ALERT_WARNING | ALERT_NOTE | ALERT_CAUTION, "zcc_main");
  }
  ...
  if (qasm_canonical_mode || qasm_validate_mode || qasm_sim_mode || qasm_opt_mode || qasm_clifford_t_mode || qasm_sim_c_mode || (input_file && strlen(input_file) >= 5 && strcmp(input_file + strlen(input_file) - 5, ".qasm") == 0)) {
    char err_buf[512] = {0};
    ZCCQasmCircuit *circ = zcc_qasm_parse_string(source, input_file, err_buf, sizeof(err_buf));
    if (!circ) {
      fprintf(stderr, "%s\n", err_buf[0] ? err_buf : "error: failed to parse QASM circuit");
      free(source);
      return 1;
    }
    if (!zcc_qasm_validate(circ, err_buf, sizeof(err_buf))) {
      fprintf(stderr, "%s\n", err_buf[0] ? err_buf : "error: semantic validation failed");
      zcc_qasm_circuit_free(circ);
      free(source);
      return 1;
    }
    if (qasm_clifford_t_mode) {
      ZCCQasmCircuit *ct_circ = NULL;
      ZCCQasmCliffordTStats ct_stats;
      memset(&ct_stats, 0, sizeof(ct_stats));
      if (zcc_qasm_transpile_clifford_t(circ, &ct_circ, NULL, &ct_stats, err_buf, sizeof(err_buf)) != 0) {
        fprintf(stderr, "%s\n", err_buf[0] ? err_buf : "error: Clifford+T quantum transpilation failed");
        zcc_qasm_circuit_free(circ);
        free(source);
        return 1;
      }
      if (zcc_verbose_flag) {
        fprintf(stderr, "Clifford+T Transpiler:\n  gates:       %zu -> %zu\n  T-count:     %zu\n  T-depth:     %zu\n  Clifford:    %zu\n  rotations:   %zu\n  equivalence: %s\n",
                ct_stats.gates_before, ct_stats.gates_after, ct_stats.t_count, ct_stats.t_depth, ct_stats.clifford_count, ct_stats.rotations_decomposed, ct_stats.equivalence_verified ? "PASS" : "N/A");
      }
      if (explicit_output_file) {
        if (zcc_qasm_emit_file(ct_circ, output_file) != 0) {
          fprintf(stderr, "error: failed to write Clifford+T QASM to '%s'\n", output_file);
          zcc_qasm_circuit_free(ct_circ);
          zcc_qasm_circuit_free(circ);
          free(source);
          return 1;
        }
      } else {
        char *canonical_str = zcc_qasm_emit_canonical(ct_circ);
        if (canonical_str) {
    ...
    if (qasm_sim_c_mode) {
      ...
      if (explicit_output_file) {
        if (zcc_qasm_emit_c_file(circ, output_file, &c_cfg, err_buf, sizeof(err_buf)) != 0) {
          fprintf(stderr, "%s\n", err_buf[0] ? err_buf : "error: failed to write standalone C simulation code");
          zcc_qasm_circuit_free(circ);
          free(source);
          return 1;
        }
      } else {
        char *c_code = zcc_qasm_emit_c_code(circ, &c_cfg, err_buf, sizeof(err_buf));
        if (!c_code) {
          fprintf(stderr, "%s\n", err_buf[0] ? err_buf : "error: failed to generate standalone C simulation code");
          zcc_qasm_circuit_free(circ);
          free(source);
          return 1;
        }
    ...
    if (qasm_sim_mode) {
      ...
      if (!sim) {
        fprintf(stderr, "error: simulator allocation failed (total qubits: %d exceeds maximum %d)\n", circ->total_qubits, ZCC_QASM_MAX_SIM_QUBITS);
        zcc_qasm_circuit_free(circ);
        free(source);
        return 1;
      }
      if (!zcc_qasm_sim_apply_circuit(sim, circ)) {
        fprintf(stderr, "error: %s\n", sim->last_error[0] ? sim->last_error : "circuit execution error");
        zcc_qasm_sim_free(sim);
        zcc_qasm_circuit_free(circ);
        free(source);
        return 1;
      }
      char *dump = zcc_qasm_sim_dump_state(sim, 1e-6);
      if (dump) {
        if (explicit_output_file) {
    ...
    if (qasm_canonical_mode || (input_file && strlen(input_file) >= 5 && strcmp(input_file + strlen(input_file) - 5, ".qasm") == 0 && !qasm_validate_mode)) {
      if (explicit_output_file) {
        if (zcc_qasm_emit_file(circ, output_file) != 0) {
          fprintf(stderr, "error: failed to write canonical QASM to '%s'\n", output_file);
          zcc_qasm_circuit_free(circ);
          free(source);
          return 1;
        }
      } else {
        char *canonical_str = zcc_qasm_emit_canonical(circ);
        if (canonical_str) {
          fputs(canonical_str, stdout);
          free(canonical_str);
        }
  ...
  zcc_oracle_log_pass("parser", "_global", (double)(p2_end - p2_start) * 1000.0 / CLOCKS_PER_SEC, 0, 0, 0, 0);
  ...
  if (g_emit_wasm) {
    ...
    if (wasm_ret == 0) {
      ...
      fprintf(stderr, "zcc: failed to emit WebAssembly module to '%s'\n", output_file);
      free(source);
      free(cc);
      ir_telem_shutdown();
      return 1;
  ...
  zcc_oracle_log_pass("constant_folding", "_global", (double)(p3_end - p3_start) * 1000.0 / CLOCKS_PER_SEC, 0, 0, 0, 0);
  ...
  zcc_oracle_log_pass("peephole", "_global", (double)(p5_end - p5_start) * 1000.0 / CLOCKS_PER_SEC, 0, 0, 0, 0);
