...
def _resolve_authoritative_bases(bases: list) -> list[Path]:
...
def _contained(resolved: Path, bases: list[Path]) -> bool:
    for base in bases:
        try:
...
def validate_safe_path(
    ...
    authoritative_safe_bases: Optional[list[Path]] = None,
) -> Path:
